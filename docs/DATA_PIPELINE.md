...
Evidence composition is not itself a reason to hold a story. One opened primary source can support `official`. One opened source from the curated `media` registry can support a bounded `media_report` when the confirmed subject, fixed-window placement and core claim are clear; if that media source explicitly relays an identifiable official announcement, interview or statement, the editor may use `media_relay_official` and state the relay chain in `verification`. Only `multi_source_verified` requires two independent reliable sources. Credible but unconfirmed reporting may publish in `rumors` with `fact_status: "unconfirmed"` and active tracking.

`needs_review` is reserved for a material blocker: unresolved subject identity, contradictory evidence that changes the claim, inability to prove the fixed-window boundary, mutable aggregate values such as current review scores/counts without the required snapshot, or evidence too incomplete to write a bounded claim. It must not be used merely because only one curated media source is open. Evidence packet `readiness` labels describe composition only. When an opened article body omits a timestamp, the evidence builder may retain the same trusted listing/RSS timestamp that originally placed that source appearance inside the fixed window; article metadata or a visible article timestamp takes precedence when available.

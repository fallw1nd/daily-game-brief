...
GitHub Actions prepares bounded evidence and deterministic state. Production Daily uses one long-lived ChatGPT task with two exact evidence-bounded editorial invocations; trusted code on `main` remains the only production publisher.
...
The ChatGPT scheduled task owns only evidence-bounded editorial work:
...
- `Brief publication SLA watchdog` has separate Daily recovery/preflight and fallback opportunities: the 11:00 Daily watchdog pass is recovery/preflight only; 11:40 is the first Daily timeout/degraded fallback deadline. Legacy AM/PM deadlines remain separate from the Daily contract.

### Final-window discovery

`Final editorial packet` starts at the fixed production evidence cutoff and captures the complete edition window. Legacy AM/PM use 10:10/17:00 Asia/Shanghai; Daily uses a 10:10 cutoff with 12:00 public `plannedAt`. Packet start time is never the ChatGPT task invocation time.
...
`scripts/editorialize.mjs` builds a compact, finalized editorial packet for the production Daily handoff. Each edition is capped at 120,000 evidence characters (roughly 30,000 reading tokens), carries the active `contractVersion: 2` decision schema, records coverage through the fixed cutoff, and is persisted on `automation/state`. Production has one long-lived Daily task with exact 10:20 and 11:20 invocations in `Asia/Shanghai`; the former PM task is disabled.

The ten-minute first-pass separation is intentional: GitHub closes the Daily evidence window and starts `Final editorial packet` at 10:10, the first ChatGPT invocation is 10:20, the second invocation is an hourly-safe 11:20 safety pass, and public `plannedAt` is 12:00. The first pass normally consumes a ready packet; if the exact packet is missing it may emit the bounded wake and must stop without polling Actions. GitHub can then persist the recovered packet for the 11:20 invocation. Neither invocation may admit post-cutoff facts.

Each due edition has one durable record at `automation/status/<edition-id>.json`. It records the fixed identity and window, the finalized packet's Git blob SHA, editorial submission/validation acknowledgement, publication commit, deployment result, and independent English/media availability. All writers update this record through bounded compare-and-retry pushes to `automation/state`; mutable `latest-am.json` / `latest-pm.json` / `latest-daily.json` pointers are convenience views, never identity authorities.
...
Daily adds one narrow liveness signal without changing recovery ownership. If the 10:20 Daily invocation has no eligible ready state, it may read current `main`, derive only the immediate successor of the latest published healthy AM or Daily edition, require that candidate Canonical to be absent, and commit one exact `automation/wake/<edition-id>.json` on `automation/editorial/<edition-id>`. The payload is fixed to schema v1, `period:"daily"`, and `reason:"packet_missing_at_handoff"`. The invocation then stops; it does not inspect or poll Actions. `.github/workflows/news-discovery-shadow.yml` accepts that push only from `automation/editorial/*-daily`, revalidates the branch edition and wake payload, and runs `resolve-packet-dispatch.mjs` with exact `period + edition`. The resolver rejects mismatched or too-early targets. Thus the wake is merely a second scheduler signal; GitHub still performs all collection, packet acknowledgement, SLA recovery, degraded publication, deployment, and incidents.

After packet persistence, GitHub may dispatch the exact edition into the Daily watchdog. The 11:00 Daily watchdog pass is recovery/preflight only: it may restore or recover the exact packet and acknowledge durable state, but it must not mark editorial `timed_out`, publish degraded Canonical, or open the timeout incident merely because no editorial decision exists yet. The same long-lived ChatGPT task gets its second exact opportunity at 11:20. 11:40 is the first Daily timeout/degraded fallback deadline; only then may an still-unhealthy edition enter the GitHub-owned degraded path. The schedule resolver always selects the oldest unpublished due window, while explicit `workflow_dispatch` remains bound to the supplied exact edition and fixed window, preventing runner-time edition drift.

For every included decision, `sharedFactFrame` is the language-neutral boundary for subject title key, dates, times, numbers, platforms, people/entities, versions, and proper terms. Simplified Chinese Canonical copy and `locales.en` must stay inside this frame. The Scheduled Task contract requires validator-parity identity checks before commit, including subject-title equality and exact platform-array equality, while the trusted publisher remains authoritative and fail-closed. English is independently edited rather than sentence-by-sentence translated; official English terminology is preferred when opened English evidence provides it. When complete English copy cannot be produced safely, the task omits `locales.en` instead of weakening or changing the Canonical decision.
...
For Daily, the 11:00 watchdog first restores the matching packet blob acknowledged by `automation/status/<edition-id>.json` and validates the complete finalized-packet contract, not only the edition ID. If the packet is missing, stale, malformed, or pre-cutoff while the edition is unhealthy, that preflight may rerun collection, ledger update, evidence extraction, and packet construction in its own Node environment. A recovered packet and its acknowledgement are committed with the same bounded compare-and-retry rule; publication does not proceed from an unacknowledged recovery packet. Legacy AM/PM watchdog times retain their own historical SLA behavior.

At or after the 11:40 Daily fallback deadline, and only while the edition is still unhealthy, the watchdog may use `scripts/build-degraded-decision.mjs`. It admits only windowed A-level events with an opened primary source or two independent opened sources, preserves source-language facts, and does not invent translations, rumors, or analysis. Its `main` publication uses the same rebuild-on-current-main retry principle for up to three attempts. If the repository already contains the edition but Pages is still unhealthy, the watchdog re-dispatches Pages rather than treating the absence of a new data diff as sufficient recovery. If collection fails or no event meets the threshold, it opens an incident instead of fabricating an edition.

### Observation and refinement

Legacy AM/PM used separate 10:20/17:10 ChatGPT tasks. After formal Daily cutover, production keeps one long-lived Daily task enabled with exact 10:20 and 11:20 invocations, while the former PM task stays disabled. The bounded wake signal is used only when the exact acknowledged packet is missing; the 11:00 GitHub pass supplies independent recovery/preflight, and the 11:40 GitHub pass supplies the fallback boundary. Production acceptance must verify normal packet availability, wake-to-second-pass liveness, trusted publication, English, media, durable state, and Pages before any timing is tightened again.

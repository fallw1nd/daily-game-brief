...
正常ChatGPT编辑未在SLA前完成时，watchdog仍使用同一份immutable packet，但不再把当日`upcomingDiscovery`全部丢弃。降级决策器只生成保守的`inherit_and_patch`补丁：候选必须来自一手官方来源、已经在名称登记中或有跨来源线索、所有发现记录对发售日期一致、日期位于本期完整15天窗口内，并且尚未存在于Canonical基线。媒体月历单独出现、未知低信号单源条目、日期冲突和已有基线都不会自动新增；缺失记录也绝不触发删除。

这一降级补丁只解决“发现成功但编辑超时导致日历完全不滚动”的可用性缺口，不把发现器升级为完整发行数据库。正常编辑路径仍需打开官方详情并可处理地区、平台、版本、抢先体验及延期等更复杂情况；降级路径宁可少收，也不猜日期或合并冲突。
...
发现层不直接写入public/data，不自动增加official事实。正常路径由编辑核验后交给可信publisher；SLA无AI降级路径仅可按上述保守条件从packet中的一手发现生成有限补丁，最终仍由同一可信publisher、双语、媒体与发布检查完成。
...
自动测试覆盖日期边界/跨年、模糊日期、平台身份、重复项、冲突日期、媒体线索分类、来源宕机、页面结构变化、体积和输入预算、真实packet构建断网，以及封面失败占位。降级发布另覆盖基线去重、一手来源采用、低信号单源排除与日期冲突fail closed。
...
最终自动检查以`npm run check`为准；历史2026-09-08初次落地时为68个测试文件、338项测试、29期数据与英文校验、生产构建。所有来源断网的真实packet构建测试通过，日历失败不会生成虚构条目。

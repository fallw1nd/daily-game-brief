...
- **Status:** in_progress
- **Evidence:** 从零审计确认两个外部 ChatGPT Scheduled Tasks 与八个 GitHub schedules 之间只通过 mutable packet path、分支观察和运行日志协作；没有逐期 packet blob、editorial validation、publication commit、deployment result 的共同耐久状态。旧 ChatGPT prompt 还允许自行检查 Actions 并动态创建 one-shot workflow，形成两个恢复 owner。后续契约复核又发现 `editorial.invalid` 不在任务选择集，且 normal Canonical 检查晚于 inbox 提交；前者会把可修复 decision 留给 degraded SLA，后者会在 `automation/state` 落后 `main` 时产生不必要的重复编辑。
- **Risk:** cron 延迟、跨日、重复执行、并发 push、旧分支提交、invalid decision 或部分成功会造成漏期、错期、重复编辑、重复恢复或“内容已提交但下游不知道”的 split-brain；可选英文/媒体失败还可能被误认为 Canonical 发布失败。
...
- **Resolution:** `codex/production-reliability-state-machine` 已实现 durable 架构；后续 Scheduled Task Contract 修复补上 invalid same-packet repair、GitHub-owned in-flight lane 隔离与生成前 Canonical 幂等检查。Daily Edition Migration 的 precutover 兼容继续复用同一 durable state/packet/publisher 模型，不建立 Daily 专属状态机：PR #60 在隔离分支新增 `am | pm | daily` 固定窗口真源、manual Daily packet/SLA/publisher/media 路径、Daily Canonical v2/English/frontend兼容，以及候选上限与120k packet budget omission telemetry；生产 AM/PM cron、外部 Scheduled Task 与历史 archive 均保持不变。正式 cadence cutover 仍需另行授权。
- **Verification:** 状态机、resolver 与 thin-prompt 契约已增加 invalid repair、lane ownership、state/main 不同步及 Canonical 检查顺序回归测试；2026-08-31 完整 `npm run check` 通过（30 个测试文件、188 项测试，data/locales/build 均通过；保留既有 historical discoveryQueries warning）。PR #60 precutover head `66797d5` 的 Verify run [33354801629](https://github.com/fallw1nd/daily-game-brief/actions/runs/33354801629) 再次通过完整 `npm run check`，新增回归覆盖 PM→Daily、Daily→next AM rollback、Daily durable state/publisher/English 与 10/20/40/80 容量压力；由于尚未正式 cutover、尚无真实 Daily production evidence，本条继续保持 `in_progress`。

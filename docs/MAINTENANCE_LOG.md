...
## 2026-09-04 append-only follow-up

### Follow-up — MNT-20260830-01

本段是既有 `MNT-20260830-01` 的追加记录，不是新问题。

- **2026-09-04 production evidence:** 唯一长期 ChatGPT Daily task 的 10:20 invocation 再次写入 exact `automation/wake/2026-09-04-daily.json`；GitHub packet run [33829110734](https://github.com/fallw1nd/daily-game-brief/actions/runs/33829110734) 随后于北京时间约10:22将 immutable `2026-09-04-daily` packet 持久化为 ready。实际 Scheduled Task 回读显示同一 task 当天 11:20 safety pass 确实再次运行；该 pass 提交的首个 editorial decision 进入 publisher run [33833166636](https://github.com/fallw1nd/daily-game-brief/actions/runs/33833166636)，但 validator 以 `decisions[26]: sharedFactFrame.platforms must match canonical platform decision` 拒绝。修正后的同 edition submission 由 normal trusted publisher run [33833858160](https://github.com/fallw1nd/daily-game-brief/actions/runs/33833858160) 成功提交 Canonical，并直接得到 `localeEn=available`，没有进入 degraded fallback。由于 durable `editorial-valid` 与 publication commit 均落在北京时间11:40之后约几十秒，本次虽然证明“10:20 wake → packet ready → 11:20 第二次模型机会 → normal publisher”链路真实存在，但没有满足既定“11:40前 normal publisher 接管”的关闭条件。
...
- **Recovery scope:** The trusted handoff writes a real `bundle-plan.json` beside packet batches and persists it with `automation/state`; GitHub rebuilds absent/stale plans from the acknowledged packet, queue and transitions. Queue order remains the fairness authority, showcase start/replay is covered, and the SLA watchdog scans due queues across editions with one bounded refresh/activation instead of filtering only the current day. Older editorial branches must use the existing single-inbox publisher path until the bundle workflow is present on trusted `main`.

...
2. Read `automation/status/<edition-id>.json`. Require schema v1 and a 40-character `packet.blobSha`. For `invalid`, require durable `validationErrors` and `submissionSha`, then read that prior decision. Fix only those errors and consequential consistency in the same edition. Never rediscover events, change packet, or advance.

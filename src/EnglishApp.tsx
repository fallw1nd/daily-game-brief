...
function storyTitle(entry: BriefEntry): string {
  return entry.title.title_en;
}
...
function editionHref(editionId: string, entryId = "top"): string {
  const params = new URLSearchParams({ edition: editionId, lang: "en" });
...
function StatusMark({ status, timeStatus }: { status: FactStatus; timeStatus: BriefEntry["time_status"] }) {
  ...
  return (
    <span className={"status-mark " + (uncertain ? "status-mark--warning" : "")}>
      {uncertain ? <WarningCircle weight="fill" aria-hidden="true" /> : <CheckCircle weight="fill" aria-hidden="true" />}
      {statusLabels[status]}
      {timeStatus === "date_only" && " / Date only"}
      {timeStatus === "time_unverified" && " / Time unverified"}
    </span>
  );
}

function PendingMark({ tracking }: Pick<BriefEntry, "tracking">) {
  return tracking === true ? <span className="pending-mark">Tracking</span> : null;
}
  ...
  return (
    <article className="lead-story" id={"lead-" + entry.id}>
      ...
      <div className="lead-story__body">
        ...
        <p>{entry.summary}</p>
        <a className="read-link" href={"#" + entry.id}>Read verification record<ArrowRight aria-hidden="true" /></a>
      </div>
    </article>
  );
}
  ...
  return (
    <a className="focus-item" href={"#" + entry.id}>
      ...
      <span className="focus-item__copy">
        ...
        <span className="focus-item__state"><PendingMark tracking={entry.tracking} /><span>{statusLabels[entry.fact_status]}</span></span>
      </span>
    </a>
  );
}
...
function StorySection({
  ...
  return (
    <section className="editorial-section" id={id} aria-labelledby={id + "-title"}>
      ...
      <div className="story-list">{entries.map((entry, index) => <StoryRow key={entry.id} entry={entry} index={index} />)}</div>
    </section>
  );
}
...
function EnglishUnavailable({ message }: { message: string }) {
  const params = new URLSearchParams(window.location.search);
  params.delete("lang");
  return (
    <div className="site-shell" data-theme={preferredTheme()} data-accent={preferredAccent()}>
      <main id="top">
        <section className="edition-masthead" aria-labelledby="page-title">
          <div className="edition-masthead__title">
            <span>DAILY GAME BRIEF</span>
            <h1 id="page-title">English version unavailable</h1>
            <p>{message}</p>
            <a className="read-link" href={import.meta.env.BASE_URL + "?" + params.toString()}>Read the Simplified Chinese edition<ArrowRight aria-hidden="true" /></a>
          </div>
        </section>
      </main>
    </div>
...
export default function EnglishApp() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const [latestResult, manifestResult, localeResult, searchResult] = await Promise.allSettled([
        loadLatestEdition(controller.signal),
        loadBriefManifest(controller.signal),
        loadEnglishLocaleIndex(controller.signal),
        loadEnglishSearchIndex(controller.signal),
      ]);
      if (controller.signal.aborted) return;
      if (manifestResult.status !== "fulfilled" || localeResult.status !== "fulfilled") {
        setLoadError("The English archive index is temporarily unavailable.");
        return;
      }
      setManifest(manifestResult.value);
      setLocaleIndex(localeResult.value);
      if (searchResult.status === "fulfilled") setSearchIndex(searchResult.value);
      ...
      if (requestedItem) {
        try {
          canonical = await loadArchivedEdition(requestedItem, controller.signal);
        } catch {
          setLoadError("The requested archive edition could not be loaded.");
          return;
        }
      }
      if (!canonical) {
        setLoadError("The canonical edition could not be loaded.");
        return;
      }
      const availability = localeResult.value.editions.find((item) => item.editionId === canonical!.id);
      if (!availability || availability.status !== "available") {
        setLoadError(availability?.status === "unavailable" ? availability.summary : "No validated English Overlay exists for this edition.");
        return;
      }
      try {
        ...
        if (!projected) {
          setLoadError("The English Overlay could not be projected safely onto the canonical edition.");
          return;
        }
        setEdition(projected);
      } catch {
        setLoadError("The validated English Overlay could not be loaded.");
      }
    })();
  ...
  const englishArchiveTitles = useMemo(() => {
    const result = new Map<string, string>();
    for (const item of localeIndex?.editions ?? []) {
      if (item.status === "available") result.set(item.editionId, item.archiveTitle);
    }
    return result;
  }, [localeIndex]);
  const archiveCounts = useMemo(() => {
    const counts = new Map<string, number>();
    for (const entry of searchIndex?.entries ?? []) counts.set(entry.editionId, (counts.get(entry.editionId) ?? 0) + 1);
    return counts;
  }, [searchIndex]);
  ...
  const visibleStorySections = storySectionDefinitions
    ...
    .map((section, index) => ({ ...section, number: String(index + 2).padStart(2, "0") }));
  ...
  if (edition.sourceReport) nextSectionNumber += 1;
  ...
  const directoryItems = [
    ...visibleStorySections.map((section) => ({ number: section.number, label: section.label, count: section.entries.length, href: "#" + section.id })),
    ...
    <div className="site-shell" data-theme={theme} data-accent={accent} ref={rootRef}>
      ...
      <header className="topbar">
        <a className="brand" href={import.meta.env.BASE_URL + "?lang=en#top"} aria-label="Daily Game Brief home">
          <span>GAME BRIEF</span><small>DAILY GAME BRIEF</small>
        </a>
        ...
        <button
          ...
          type="button"
          aria-label={menuOpen ? "Close directory" : "Open directory"}
      ...
      <main id="top">
        ...
        <section className="lead-desk masthead-reveal" id="today" aria-labelledby="lead-title">
          <header className="desk-label"><span>01</span><h2 id="lead-title">Today&apos;s focus</h2><small>EDITOR&apos;S ORDER</small></header>
          {focusEntries[0] ? (
          ...
          ) : <p className="empty-line">No focus item in this edition.</p>}
        </section>

        {directoryItems.length > 0 && (
          <div className="edition-directory" aria-label="Non-empty sections">
            {directoryItems.map((item) => (
              <a key={item.href} href={item.href}><span>{item.number}</span><strong>{item.label}</strong><small>{String(item.count).padStart(2, "0")}</small></a>
            ))}
          </div>
        )}
        ...
        <div className="edition-content" id="content">
          {visibleStorySections.map((section) => (
            <StorySection key={section.id} id={section.id} number={section.number} title={section.title} entries={section.entries} note={section.note} />
          ))}
          ...
          {edition.sourceReport && (
            <section className="editorial-section source-report" id="source-report" aria-labelledby="source-report-title">
              <SectionHeader number={sourceReportNumber} title="Source review" count={edition.sourceReport.checked.length} id="source-report-title" />
              <div>
                <section><h3>Checked</h3><p>{edition.sourceReport.checked.join(" / ")}</p></section>
                <section><h3>Access limited</h3><p>{edition.sourceReport.limited.join(" / ") || "None"}</p></section>
                <p>{edition.sourceReport.note}</p>
              </div>
            </section>
          )}
        </div>
        ...
        <section className="archive-section" id="archive" aria-labelledby="archive-title">
          <header>
            ...
            <div>
              <h2 id="archive-title">Brief archive</h2>
              <p>{manifest ? `${manifest.editions.length} editions archived. Browse by edition or search across validated English copy.` : "Loading archive index."}</p>
            </div>
            <label className="search-field">
              <span>Search games, platforms or events across English editions</span>
              <div><MagnifyingGlass aria-hidden="true" /><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search the archive" /></div>
            </label>
          </header>

          {query.trim() && (
            <section className="archive-search" aria-labelledby="archive-search-title">
              <header className="archive-subhead"><h3 id="archive-search-title">Search results</h3><span>{searchResults.length} matches</span></header>
              ...
              {searchResults.length > visibleSearchResults.length && <p className="archive-status">Showing the first 100 matches. Narrow the query for more precise results.</p>}
            </section>
          )}

          <section className="archive-editions" aria-labelledby="archive-editions-title">
            <header className="archive-subhead"><h3 id="archive-editions-title">All editions</h3><span>{archiveEditions.length} EDITIONS</span></header>
            <div className="archive-edition-list" id="archive-edition-list">
              {visibleArchiveEditions.map((item) => {
                const title = englishArchiveTitles.get(item.id);
                const available = Boolean(title);
                return available ? (
            ...
            {hiddenArchiveCount > 0 && (
              <button className="archive-toggle interaction-state" type="button" aria-expanded={archiveExpanded} aria-controls="archive-edition-list" onClick={() => setArchiveExpanded((expanded) => !expanded)}>
                <span>{archiveExpanded ? "Collapse to the latest 5 editions" : `Show ${hiddenArchiveCount} more editions`}</span><CaretDown aria-hidden="true" />
              </button>
            )}
          </section>
        </section>

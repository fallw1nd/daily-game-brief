...
const instructions = [
  ...
  "locales.en.entries 必须按 include 决定的顺序，以 eventKey 一一对应；locales.en.upcoming 只覆盖本次实际提交的 upcoming patch，并按 upcomingId 一一对应；由 trusted publisher 继承的既有日历项不需要重复生成英文 patch。英文 archiveTitle：早报以 'Morning Brief |' 开头，晚报以 'Evening Brief |' 开头，日报以 'Daily Brief |' 开头。",
  ...
  "日报的 editorialInput.upcomingBaseline.refreshRange 是事件 packet-only 规则的唯一日历例外，每天覆盖完整未来15天，不把旧期次存在等同于已完成核验。只对 startInclusive 到 endInclusive 做发售日历核验：优先开发商/发行商官方公告、PlayStation/Nintendo/Xbox/Steam 官方商店或平台页面，并打开每个采用的来源。查到的事实只能写入 upcoming 日历字段，不能改变 packages/trackingQueue 的 include/exclude、正文事实、时间、factStatus、source classification 或 tracking。若没有可靠来源则不要填充。",
  "editorialInput.upcomingDiscovery 是自动发现线索，不是可直接发布的事实。优先核验 knownTitle、crossSource 和 inBaseline=false 的记录；所有采用项都要打开官方详情页，核对日期、平台、地区、正式版/抢先体验/移植/DLC及完整游戏身份。dates有冲突时不能猜选；reviewLinks是待阅读文章，不是游戏条目。对PC、PlayStation、Xbox、Nintendo分别搜索完整15天；coverage中failed/empty_or_changed或只有部分列表、omittedCandidates>0必须做针对性补查，并在sourceReport记录实际检查与缺口；不得声称全量覆盖。只修改日历字段，不扩充新闻事件。",

...
const pipeline = await readFile("docs/DATA_PIPELINE.md", "utf8");
...
describe("Daily rolling upcoming contract", () => {
  it("uses inherit-and-patch instead of destructive replacement", () => {
    expect(contract).toContain('Daily uses `upcomingMode:"inherit_and_patch"`');
    expect(pipeline).toContain("carries the newest verified Canonical calendar forward");
    expect(pipeline).toContain("strict future-15-day range");
  ...
  it("restores the baseline in trusted publisher code", () => {

...
export const DEFAULT_EVIDENCE_CANDIDATE_LIMIT = 30;
const USER_AGENT = "DailyGameBriefEvidenceBot/1.0 (+https://fallw1nd.github.io/daily-game-brief/)";

export function candidateLimitTelemetry(reviewQueue = [], limit = DEFAULT_EVIDENCE_CANDIDATE_LIMIT) {
  const normalizedLimit = Math.max(0, Number(limit) || 0);
  const omitted = reviewQueue.slice(normalizedLimit);
  return {
    reviewQueueCandidates: reviewQueue.length,
    candidateLimit: normalizedLimit,
    selectedCandidates: Math.min(reviewQueue.length, normalizedLimit),
    omittedByCandidateLimit: omitted.length,
...
async function main() {
  const maxCandidates = Number(process.env.EVIDENCE_CANDIDATE_LIMIT || DEFAULT_EVIDENCE_CANDIDATE_LIMIT);
  ...
  const candidateTelemetry = candidateLimitTelemetry(report.reviewQueue || [], maxCandidates);
  const selectedCandidates = (report.reviewQueue || []).slice(0, maxCandidates);
  ...
  const output = {
    ...
    limits: { candidatePackages: maxCandidates, sourcesPerCandidate: 3, evidenceCharsPerSource: MAX_EVIDENCE_CHARS },
    candidateTelemetry,
    totals: {
  ...
  console.log(`Evidence packages: ${output.totals.packages}; ready=${output.totals.primaryPlusIndependent}; limited pages=${output.totals.limitedPages}; candidate-limit omitted=${candidateTelemetry.omittedByCandidateLimit}; omitted A=${candidateTelemetry.omittedTierA}`);
